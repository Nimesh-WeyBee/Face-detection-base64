...
Promise.all([
  faceapi.nets.ssdMobilenetv1.loadFromDisk(
    path.join(MODELS_URL, "ssd_mobilenetv1_model")
  ),
  faceapi.nets.faceLandmark68Net.loadFromDisk(
    path.join(MODELS_URL, "face_landmark_68_model")
  ),
  faceapi.nets.faceRecognitionNet.loadFromDisk(
    path.join(MODELS_URL, "face_recognition_model")
  ),
]).then(() => {
...
app.post("/crop-face", async (req, res) => {
  ...
  try {
    ...
    if (!detection || !detection.descriptor) {
      return res
        .status(400)
        .json({ message: "No face detected in the ID card." });
    }
    ...
    if (faceImages.length > 0) {
      const out = fs.createWriteStream(
        path.join(__dirname, "faces", "reference.png")
      );
    ...
    console.log(
      "Reference descriptor saved with length:",
...
app.post("/compare-face", async (req, res) => {
  ...
  if (!fs.existsSync(descriptorPath)) {
    return res.status(400).json({
      message: "Reference face not found. Please run /crop-face first.",
    });
  }
  ...
  try {
    ...
    if (!detection || !detection.descriptor) {
      return res
        .status(400)
        .json({ message: "No face detected in the given image." });
    }
...
app.post("/compare-faces", async (req, res) => {
  ...
  if (!image1 || !image2) {
    return res.status(400).json({ message: "Two images must be provided." });
  }
  ...
  try {
    ...
    const descriptor1 = await getDescriptor(image1);
    const descriptor2 = await getDescriptor(image2);
    ...
    return res.status(500).json({
      message: err.message || "Error comparing faces.",
    });
  }
});
